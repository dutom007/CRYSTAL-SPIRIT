...
class AdminPspagebuilderFooterController extends ModuleAdminController {
		...
		if ($this->profile->id)
		{
			...
			$this->page_header_toolbar_btn['duplicate'] = array(
				...
				'js' => 'if (confirm(\''.$this->l('Are you sure to copy this?', null, true, false).' ?\')) document.location = \''.
						$this->context->link->getAdminLink('AdminPspagebuilderFooter').'&token='.$token.'&id_pagebuilderprofile='.
						(int)$this->profile->id.'&duplicateprofile\'; else document.location = \''.
						$this->context->link->getAdminLink('AdminPspagebuilderFooter').
	...
	public function postProcess()
	{
		...
		$tabAccess = Profile::getProfileAccess($this->context->employee->id_profile, Tab::getIdFromClassName('AdminPspagebuilderFooter'));
	...
	public function ajaxDoListwidgets()
	{
		...
		$widgets = $this->profile->getButtons('is_footer');
