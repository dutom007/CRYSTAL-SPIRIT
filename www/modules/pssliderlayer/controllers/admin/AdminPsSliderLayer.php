...
class AdminPsSliderLayerController extends ModuleAdminController
{
	...
	{
		$this->addCss(__PS_BASE_URI__.str_replace('//', '/', 'modules/pssliderlayer').'/views/css/admin/admincontroller.css', 'all');
		//_PS_THEME_DIR_
	...
	public function getSlidesByGroup($id_group)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                    SELECT lsll.`id_lang`, lsl.`id_pssliderlayer_slides` as id, lsl.*,lsll.*
	...
	public function renderList()
	{
		...
		if ($typo)
		{
			...
			if (file_exists(_PS_THEME_DIR_.'/assets/css/modules/pssliderlayer/views/css/typo.css'))
				$typoDir = _THEME_DIR_.'css/'.str_replace('//', '/', '/assets/modules/pssliderlayer').'/views/css/typo.css';
			else
				$typoDir = __PS_BASE_URI__.str_replace('//', '/', 'modules/pssliderlayer').'/views/css/typo.css';

			$this->addCss($typoDir, 'all');
			$this->addJS(__PS_BASE_URI__.'modules/pssliderlayer/views/js/admin/jquery-ui-1.10.3.custom.min.js');
		...
		$image_uploader->setMultiple(true)->setUseAjax(true)->setUrl(
				Context::getContext()->link->getAdminLink('AdminPsSliderLayer').'&ajax=1&action=addSliderImage');

		Media::addJsDef(array(
			'imgManUrl' => Context::getContext()->link->getAdminLink('AdminPsSliderLayer'),
		));
