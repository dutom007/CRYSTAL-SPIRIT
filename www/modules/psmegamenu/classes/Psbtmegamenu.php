	...
	public function getFrontTree($parent = 1, $edit = false, $params = array())
	{
		...
		if ($this->hasChild($parent))
		{
			...
			foreach ($data as $menu)
			{
				...
				//Martin - 2017 05 11 - Ajout d'une class selected en fonction de l'id de la categorie
				...
				//End - 2017 05 11 - Ajout d'une class selected
				...
				if ($this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html' || is_numeric($menu['type_submenu']))
				{
                    ...
                    <a class="has-subhtml'.(($selected) ? ' selected' : '' ).'" href="'.$this->getLink($menu).'" target="'.$menu['target'].'">';

                    // span image + font-icon
					if ($menu['icon_class'])
						$output .= '<h1 class="hasicon"><i class="material-icons">'.$menu['icon_class'].'</i>';
					elseif ($menu['image'] && is_file(_PSMEGAMENU_IMAGE_DIR_.$menu['image']))
						$output .= '<h1 class="hasicon menu-icon"><img src="'.$this->image_base_url.$menu['image'].'" alt=""/>';
					if (($menu['image'] && is_file(_PSMEGAMENU_IMAGE_DIR_.$menu['image'])) || $menu['icon_class'])
						$output .= '</h1>';

					// span title - sub-title - description
					$output .= '<h1 class="menu-title'.(isset($menu['show_title']) && !$menu['show_title'] ? ' hidden' : '').'">'.$menu['title'].'</h1>';
					if ($menu['text'])
						$output .= '<h1 class="sub-title">'.$menu['text'].'</h1>';
					if ($menu['description'])
						$output .= '<h1 class="menu-desc">'.$menu['description'].'</h1>';
				...
				else
				{
                    ...
                    <a '.(($selected) ? 'class="selected" ' : '' ).'href="'.$this->getLink($menu).'" target="'.$menu['target'].'">';
