...
if (!class_exists('PtsThemeSample'))
{
	...
	class PtsThemeSample {
		...
		public function exportDBStruct()
		{
			...
			$first_insert_table = array(
				_DB_PREFIX_.'attribute_group',
				_DB_PREFIX_.'shop_group',
				_DB_PREFIX_.'shop',
				_DB_PREFIX_.'lang', 
			...
			foreach ($tables as $k=>$table) {
				...
				if (in_array($schema[0]['Table'], $first_insert_table)) {
					unset ($tables[$k]);
				}
			}
		
			foreach ($first_insert_table as $k => $value) {
				$tmp[$k] = array( $key => $value);
			}

			$new_tables = array_merge( $tmp, $tables );
			...
			foreach ($new_tables as $table)
			{
				...
				if (!in_array($schema[0]['Table'], $ignore_insert_table)){
					//$data .= '-- '.$found.' `'.$table.'`;'.PHP_EOL;
					$data .= 'DROP TABLE IF EXISTS `'.$table.'`;'.PHP_EOL;
			...
			// replace innodb
			$data1 = str_replace('ENGINE=InnoDB', 'ENGINE=ENGINE_TYPE', $data);

			// replace ps_ prefix
			$data2 = str_replace('`ps_', '`PREFIX_', $data1);

			// remove auto increment
			$partern = "/AUTO_INCREMENT=[0-9]+/";
			$rs = preg_replace($partern, '', $data2);
		...
		public function exportThemeSql()
		{
			...
			$ignore_insert_value_table = array(
			...
			if (!is_dir($install_folder))
				mkdir($install_folder, 0755);
			$backupfile = $install_folder.'/db_data.sql';
			...
			foreach ($new_tables as $table)
			{
				...
				// theme.sql
				if (!in_array($schema[0]['Table'], $ignore_insert_value_table))
				{
					$sql .= PHP_EOL.'TRUNCATE TABLE '.str_replace('`'._DB_PREFIX_, '`PREFIX_', '`'.$schema[0]['Table']).'`;'.PHP_EOL;
					$data_sql = Db::getInstance()->Executes('SELECT * FROM `'.$schema[0]['Table'].'`');
					$sizeof = DB::getInstance()->NumRows();
					$lines = explode("\n", $schema[0]['Create Table']);

					if ($data_sql && $sizeof > 0)
					{
						// Export the table data
						$sql .= PHP_EOL.'INSERT INTO '.str_replace('`'._DB_PREFIX_, '`PREFIX_', '`'.$schema[0]['Table']).'` VALUES'.PHP_EOL;
		...
		public static function assignHooksModulesInXml($theme, $modules)
		{
			...
			// end get array node hook, position, exceptions xml modules

			//echo "<pre>"; print_r($module_hook); die;
		...
		public function importModule($module, $mod = 'config')
		{
			...
			if (isset($xml_modules[$module]['images']) && isset($xml_modules[$module]['imgfolder']))
			{
				...
				if( isset($xml_modules[$module]['images']) && isset($xml_modules[$module]['images']['image']) ){
