...
class PtsthemePanel extends Module {
	...
	public function hookActionFrontControllerSetMedia(){
		...
		$this->context->controller->registerJavascript('corejs', '/themes/core.js', ['position' => 'head', 'priority' => 0]);


       
        $this->context->controller->registerJavascript('theme-main', '/assets/js/theme.js', ['position' => 'bottom', 'priority' => 50]);
        $this->context->controller->registerJavascript('theme-custom', '/assets/js/custom.js', ['position' => 'bottom', 'priority' => 1000]);
	...
	public function getThemes()
	{
		if ($this->themes)
			return $this->themes;
		$themes = array();
		$directories = glob(_PS_ALL_THEMES_DIR_.$this->theme_name.'/assets/css/themes/*.css');
		$preview = 'https://placeholdit.imgix.net/~text?txtsize=33&w=220&h=280&txt=Theme';
		$themes[] = array('skin' => 'default',
			'rehook' 	=> PtsThemeSample::isReHookBySkin($this->theme_name, 'default'),
			'preview'   => $this->context->shop->getBaseURL().'themes/'.$this->theme_name.'/preview.png',
			'name' => $this->l('Default'));
		if ($directories)
		{
			foreach ($directories as $dir)
			{
				...
				if(file_exists(_PS_THEME_DIR_.'/assets/img/skins/'.$skin.'.jpg')) {
					$preview = $this->context->shop->getBaseURL().'themes/'.$this->theme_name.'/assets/img/skins/'.$skin.'.jpg';
				} else {
					$preview = 'https://placeholdit.imgix.net/~text?txtsize=33&w=220&h=280&txt=Theme';				
				}
				$themes[] = array('skin' => $skin, 'rehook' => $rehook, 'name' => $this->l(Tools::ucfirst($skin)) ,'preview' => $preview );
			}
		}
		return $themes;
	}
	...
	{ 
		...
		if ( Tools::getValue('configure') == $this->name){
			...
			$this->context->controller->addJS($this->_path.'views/js/scripts-admin.js' , 999 );
		}
	}
	...
	public function hookDisplayBackOfficeTop()
	{
		...
		$output = '
			<div id="pts-dropdown-top" class="pull-xs-left list-unstyled ps-dropdown dropdown pull-left">
					<span data-toggle="dropdown">'.$this->l('Theme Control').' <i class="material-icons arrow-down">keyboard_arrow_down</i></span>
					<ul class="ps-dropdown-menu dropdown-menu">';

		$output .= '	<li class="dropdown-item"><a href="'.$themecontrol.'">'.$this->l('Theme Control Panel').'</a></li>';
		...
		if ((int)Validate::isLoadedObject(Module::getInstanceByName('ptspagebuilder')))
		{
			...
			$output .= ' <li class="dropdown-item"><a href="'.$pagebuilder.'">'.$this->l('Page Builder Profiles').'</a></li>';
			$output .= ' <li class="dropdown-item"><a href="'.$footerbuilder.'">'.$this->l('Footer Builder Profiles').'</a></li>';
		}
		...
		{
			$output .= '	<li class="dropdown-item"><a href="'.$megamenu.'">'.$this->l('Megamenu').'</a></li>';
		}
		...
		{
			$output .= '	<li class="dropdown-item"><a href="'.$megavermenu.'">'.$this->l('Vertical Megamenu').'</a></li>';
			$output .= '	<li class="dropdown-item"><a href="'.$livevermega.'">'.$this->l('Live Vertical Megamenu Editor').'</a></li>';
		}
		$output .= '	<li class="dropdown-item"><a href="'.$livetheme.'">'.$this->l('Live Theme Editor').'</a></li>';
		$output .= '</ul></div>';
	...
	{
		$this->context->controller->registerStylesheet('theme-main', '/assets/css/'.$skinfilename, ['media' => 'all', 'priority' => 0]);
	}
	...
	{
		$theme_json = Tools::file_get_contents(_PS_CONFIG_DIR_.'themes/'.$this->theme_name.'/shop'.$this->context->shop->id.'.json');
		$theme_data = Tools::jsonDecode($theme_json,true);
		...
		if (!empty($cgtheme) && $cgtheme !='default' )
		{
		 
			$themeskinfile = _PS_ALL_THEMES_DIR_.$this->theme_name.'/assets/css/theme-'.$cgtheme.'.css';
			$themeskinfilertl = _PS_ALL_THEMES_DIR_.$this->theme_name.'/assets/css/theme-rtl-'.$cgtheme.'.css';

			if ($this->context->language->is_rtl && file_exists($themeskinfilertl))
			{
				$this->reloadGlobalCssFile('theme-rtl-'.$cgtheme.'.css');
				$loadrlt = false;
			}
			elseif (file_exists($themeskinfile)){
				$this->reloadGlobalCssFile('theme-'.$cgtheme.'.css');
			}
				
		}

		if ($this->context->language->is_rtl && $loadrlt){
			$this->reloadGlobalCssFile('rtl-theme.css');
		}
		...
		if ($this->checkVisiable())
		{
			...
			$this->theme_customize_path = _PS_ALL_THEMES_DIR_.$this->theme_name.'/assets/css/profiles/';
		}
		if ((int)Configuration::get('PTS_CP_ACTIVE') == 1 &&   $this->checkValidToken() )
		{
			...
			if (Tools::isSubmit('submitLiveThemeLayout') 
					&& Tools::getValue('index_layout')
					&& Tools::getValue('category_layout')
					&& Tools::getValue('product_layout')
					&& Tools::getValue('contact_layout')
				){
				$theme_data['theme_settings']['layouts']['index'] = Tools::getValue('index_layout');
				$theme_data['theme_settings']['layouts']['category'] = Tools::getValue('category_layout');
				$theme_data['theme_settings']['layouts']['product'] = Tools::getValue('product_layout');
				$theme_data['theme_settings']['layouts']['contact'] = Tools::getValue('contact_layout');

				$json = Tools::jsonEncode($theme_data);
				Tools:file_put_contents(_PS_CONFIG_DIR_.'themes/'.$this->theme_name.'/shop'.$this->context->shop->id.'.json', $json);
				header("Refresh:0");
			}
			...
			if (Tools::isSubmit('submitPtsLiveConfiguratorInActiveProfile')  ){
				Configuration::updateValue('PTS_CP_PROFILE', '');
			}
			...
			{ 
				...
				$fonts = PtsThemePanelHelper::loadGoogleFonts( $this->module_path.'views/js/fonts.json' ); 
				$sfonts = array();
					...
					foreach ($customizes as $key => $customize)
						if (isset($matches[$match]) && isset($matches[$match][$key]))
						{
							...
							if (trim($customize))
								...
								if( $tmp[1] == 'font-family' && !( empty($customize) || $customize == 'inherit' ) ){
									$fw  = '' ;
									if( isset($fonts[$customize]) ){
										$fw = implode(",", $fonts[$customize]['fw']);
									}
									$sfonts[] = $customize  . ':' . $fw;
								}
						}
				...
				if( !is_dir($this->theme_customize_path) ){
					@mkdir($this->theme_customize_path, 0777, true);
				}
				if ($data['action-mode'] != 'save-delete')
				{
					$cache['profile_colors'] = array();
					if( isset( $cache['body']) ){
						foreach( $cache['body'] as $key => $selector ){
							if( preg_match( '#color#', $selector['attr'] ) ){  
							  	$selector['val'] =  $selector['val'] ? '#'. $selector['val'] : '#FFFFF' ;
								$cache['profile_colors'][$key] = $selector;
							}
					...
					if( !empty($sfonts) ){
						PtsThemePanelHelper::writeToCache($this->theme_customize_path, $name_file.'-fonts', Tools::jsonEncode($sfonts), 'json');
					}
		...
		if (Configuration::get('PTS_CP_PROFILE')){ 
			...
			$this->context->controller->registerStylesheet( 'custom-profile', '/assets/css/profiles/'.Configuration::get('PTS_CP_PROFILE').'.css', 'all');
			$file = _PS_ALL_THEMES_DIR_.$this->theme_name.'/assets/css/profiles/'.Configuration::get('PTS_CP_PROFILE').'-fonts.json';
			if( file_exists($file) ){
				$content = Tools::jsonDecode( Tools::file_get_contents( $file ) );
				if( !empty($content) ){
					foreach( $content as $font ){
						$output .= '<link href="//fonts.googleapis.com/css?family='.$font.'" data-noprefix rel="stylesheet" type="text/css">'."\r\n";
					}
				}
			}
		}
		...
		if ($customjs = $this->getConfigValue('customjs'))
			$output .= '<script type="text/javascript">'.$customjs.'</script>';
	...
	{
		return (int)Configuration::get('PTS_CP_ACTIVE') == 1 && $this->checkValidToken() || _PS_MODE_DEMO_;
	}
	...
	{ 
		...
		if ($this->checkVisiable())
		{
			...
			foreach ($files as $dir){
				...
				$profiles[$file] = array();
				if( file_exists($this->theme_customize_path.$file.'.json') ){ 
					$content = Tools::file_get_contents( $this->theme_customize_path.$file.'.json' );
					if( $content ){
						$json 	= Tools::jsonDecode( $content );
						if( isset($json->profile_colors) ){ 
							$profiles[$file] = $json->profile_colors;
						}
					}
			...
			$this->smarty->assign(array(
				...
				'backgroundImageURL' => $this->context->shop->getBaseURL().'themes/'.$this->theme_name.'/assets/img/patterns/'
			));

			$html .= $this->display(__FILE__, 'live_configurator.tpl');
			$xmlselectors = PtsThemePanelHelper::renderEdtiorThemeForm($this->theme_name);

		//	echo '<pre>'.print_r( $xmlselectors , 1 );die;
			$this->smarty->assign(array(
				'customizeFolderURL' => $this->context->shop->getBaseURL(true).'themes/'.$this->theme_name.'/assets/css/profiles/',
	...
	public function getContent()
	{
		...
		$btnnavs[] = array('icon' => 'fa-cogs', 'title' => $this->l('Theme Setting'));
		if ((int)Validate::isLoadedObject($module = Module::getInstanceByName('ptspagebuilder')))
			$btnnavs[] = array('icon' => 'fa-cogs', 'title' => $this->l('Page, Footer Builder'));
		$btnnavs[] = array('icon' => 'fa-cogs', 'title' => $this->l('Customization'));
		$btnnavs[] = array('icon' => 'fa-cogs', 'title' => $this->l('Modules Setting'));
	...
	public function renderConfigurationForm()
	{
			...
			if (isset($module['is_module']) && $module['is_module'])
			{
				...
				if (Validate::isLoadedObject($module_instance) && method_exists($module_instance, 'getContent'))
					$desc = '<a class="btn btn-default" href="'.$this->context->link->getAdminLink('AdminModules', true).'
						...
						'.$this->l('Configure').' <i class="fa-external-link"></i></a>';
			}
		...
		$fields_form[] = array(
			'form' => array(
                ...
                <br><a href="'.$live_theme_url.'" class="btn-danger btn"><i class="fa-external-link-sign"></i> '.
	...
	public function getList()
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE id_shop = '.(int)$id_shop.' and is_footer !=1 and is_menu !=1
		');
	}
	...
	protected function renderProfilesForm()
	{
		...
		foreach ($skins as $skin)
		{
			$inputs[] = array(
				'type' => 'select',
				'label' => $this->l('Home Page Profile For Skin').': '.$skin['name'],
				'name' => $skin['skin'].'pbuilder',
				'options' => array('query' => $pagebuilder,
					'id' => 'key',
					'name' => 'name'),
				'default' => '',
				'desc' => $this->l('Select a page builder profile as default home page')
			);
			$inputs[] = array(
				'type' => 'select',
				'label' => $this->l('Footer Profile For Skin').': '.$skin['name'],
	...
	public function checkValidToken()
	{
		...
		$token = Tools::getValue('live_configurator_token'); 
		$context = Context::getContext();
		$id_employee = $context->cookie->__get('liveecookie_eid');
 
		if( empty($token) && $id_employee ){
			$name  = 'ptsthemepanel';
			$token = Tools::getAdminToken( $name.(int)Tab::getIdFromClassName($name) . $id_employee );	
		}
	...
	public function getLiveConfiguratorToken()
	{
		...
		if( Tools::getValue('id_employee') || Tools::getValue('controller') == 'AdminModules' ) {

			$token = Tools::getAdminToken($name.(int)Tab::getIdFromClassName($name)
										.(is_object(Context::getContext()->employee) ? (int)Context::getContext()->employee->id :
														Tools::getValue('id_employee')));



			$context->cookie->__set('liveecookie_eid', Tools::getValue('id_employee') );  
			$context->cookie->__set('liveecookie', $token);
			$this->tokens[] = $token;
		}
