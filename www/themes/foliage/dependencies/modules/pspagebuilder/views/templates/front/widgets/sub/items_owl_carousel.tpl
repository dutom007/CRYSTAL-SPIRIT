...
<div class="widget-content owl-carousel-play" data-ride="owlcarousel">
	...
	{foreach from=$items item=item name=item_name}
		<div class="item ajax_block_product">
			{include file="{$list_mode_tpl}" product=$item class="products-block  {if isset($list_mode) && $list_mode == 'grid'}{if isset($product_style) && !empty($product_style)}{$product_style}{else}style1{/if}{else}list{/if}"}
		</div>
	{/foreach}
	</div>
</div>