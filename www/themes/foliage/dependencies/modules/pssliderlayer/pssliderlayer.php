...
class PsSliderLayer extends Module {
	...
	public function processAJax() {
		...
		if (Tools::getValue('action') && Tools::getValue('action') == 'submitslider') {
			...
			$myLink = AdminController::$currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules').'&ditSlider=1&id_slide=' . $slide->id . '&id_group=' . $slide->id_group;
...
//                    ),
	...
	private function _prepareHook($hookName, $group = null) {
		...
		if (!$this->isCached($tpl, $this->getCacheId($hookName . '_' . $this->name))) {
			...
			$whiteMainImg = __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/front_img/white50.png';
	...
	public function _processHook($hookName, $group = null) { 
		...
		$url = $this->context->controller->getAssetUriFromLegacyDeprecatedMethod( $this->_path . 'views/js/jquery.themepunch.revolution.js' );  
		$url2 = $this->context->controller->getAssetUriFromLegacyDeprecatedMethod( $this->_path . 'views/js/jquery.themepunch.tools.min.js' );  

		$this->context->controller->registerJavascript('themepunch2', $url, ['position' => 'head', 'priority' => 8]);
		$this->context->controller->registerJavascript('themepunch3',$url2, ['position' => 'head', 'priority' => 9]);
	...
	public function hookdisplayHeader() {
		return $this->_processHook('header');
	}
	...
	public function headerHTML() {
		...
		if (file_exists(_PS_THEME_DIR_ . 'modules/pssliderlayer/views/css/typo.css'))
			$this->context->controller->addCSS(_PS_THEME_DIR_ . 'modules/pssliderlayer/views/css/typo.css');
		else
			$this->context->controller->addCSS($this->_path . 'modules/pssliderlayer/views/css/typo.css');
