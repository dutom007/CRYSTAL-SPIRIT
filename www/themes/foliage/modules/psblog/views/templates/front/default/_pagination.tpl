...
{if isset($p) AND $p}
	...
	<!-- Pagination -->
	<div id="pagination{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination col-md-8 clearfix">
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination_previous">
				<a {$no_follow_text|escape:'html':'UTF-8'} href="{$link->goPage($requestPage, $p_previous)|escape:'html':'UTF-8'}">
					<i class="material-icons">navigate_before</i> {l s='Previous' d='Shop.Theme'}</a>
			</li>
		{else}
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="disabled pagination_previous"><span><i class="material-icons">navigate_before</i> {l s='Previous' d='Shop.Theme'}</span></li>
		{/if}
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination_next"><a {$no_follow_text|escape:'html':'UTF-8'} href="{$link->goPage($requestPage, $p_next)|escape:'html':'UTF-8'}">{l s='Next' d='Shop.Theme'} <i class="material-icons">navigate_next</i></a></li>
		{else}
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="disabled pagination_next"><span>{l s='Next' d='Shop.Theme'} <i class="material-icons">navigate_next</i></span></li>
		{/if}
		</ul>
	{/if}
	</div>
    
    <!-- <div class="product-count col-md-4 hidden-sm-down">
    	{if ($n*$p) < $nb_items }
        ...
        {/if}
        {if $nb_items > 1}
        	{l s='Showing %1$d - %2$d of %3$d items' d='Shop.Theme' sprintf=[$blogShowingStart, $blogShowing, $nb_items]}
		{else}
        	{l s='Showing %1$d - %2$d of 1 item' d='Shop.Theme' sprintf=[$blogShowingStart, $blogShowing]}
       	{/if}
    </div> -->
